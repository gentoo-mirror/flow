...
BUILD_VER="3.6.2"
BUILD_ID="I20110210-1200"
ECLIPSE_BUILD_VER="0.6.1"
...
OSGI_DEPENDENCIES=(
	...
	'org.mortbay.jetty.server jetty-server-6 jetty'
	'org.mortbay.jetty.util jetty-server-6 jetty-util'
...
eclipse_create-osgi-dep() {
	...
	if (( ${#} )) ; then
		local jar ; for jar in "${@}" ; do
			if [[ ${package} == jetty* ]]; then
				java-pkg_jar-from --virtual "${package}" "${jar}.jar"
			else
